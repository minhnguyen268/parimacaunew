...
import { LanguageSelector } from "../homePage/Header";
...
const NotAllowManagerRoutes = ["/admin/settings/telegram", "/admin/settings/tawk-to", "/admin/settings/slide"];
...
const Navbar = (props) => {
  ...
  const listItemValue =
    session?.user?.role === "admin" ? listItem : listItem.filter((item) => !NotAllowManagerRoutes.includes(item.key));
          ...
          >
            <div>
              <Typography
                component="div"
                sx={{
                  height: isExpandMenu ? "5rem" : "7rem",
                  marginBottom: !isExpandMenu ? "0.5rem" : "0",
                  cursor: "pointer",
                  display: "flex",
                  flexDirection: isExpandMenu ? "row" : "column",
                  alignItems: "center",
